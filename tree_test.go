...
func TestCarryQuery(t *testing.T) {
	entries := []listEntry{
		{Name: "docs", Route: "/_/dir/docs", Children: []listEntry{{Name: "guide.md", Route: "/_/dir/docs/guide.md"}}},
		{Name: "no route"},
	}
	carryQuery(entries, "list=scope:tree&outline=style:nh")

	if want := "/_/dir/docs?list=scope:tree&outline=style:nh"; entries[0].Route != want {
		t.Errorf("route = %q, want %q", entries[0].Route, want)
	}
	if want := "/_/dir/docs/guide.md?list=scope:tree&outline=style:nh"; entries[0].Children[0].Route != want {
